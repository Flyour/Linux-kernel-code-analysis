	...
	int write_access, pte_t * pte)
{
	...
	spin_lock(&mm->page_table_lock);
	entry = *pte;
	if (!pte_present(entry)) { /* pte_present()测试一个表项所映射的页面是否在内存中 */
		/*
		 ...
		 */
		spin_unlock(&mm->page_table_lock);
		if (pte_none(entry)) /* pte_none()测试一个表项是否为空 */
	...
	unsigned long address, int write_access)
{
    ...
    /* 根据给定的mm_struct,和具体的地址，由宏操作pgd_offset()计算出指向该地址说属页面目录项的指针。pgd_offset()在/include/asm_i386/pgtable.h中定义 */
	pgd = pgd_offset(mm, address);

    /*
     * 由于i386只使用两层映射，所以在include/asm_i386/pgtable_2level.h中将其定义为"return (pmd_t*)pgd;",也就是说，
     * 把具体的目录项当成一个只含一个表项的中间目录。
     */
	pmd = pmd_alloc(pgd, address);

	if (pmd) {
		pte_t * pte = pte_alloc(pmd, address); /* 通过pte_alloc函数，找到（或者分配）一个页面表,并找到相应的页面表项 */
		if (pte)
			ret = handle_pte_fault(mm, vma, address, write_access, pte); /* handle_pte_fault定义在 mm/memory.c 中 */
	}
	return ret;
}
